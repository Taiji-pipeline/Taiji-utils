...
def spectral_fit(args):
    model = Spectral(n_dim=args.dim, distance=args.distance, sampling_rate=args.sampling_rate)
    ...
    model.fit(mat)
    compressed_pickle(args.output, model)

def spectral_predict(args):
    model = decompress_pickle(args.model)
    print("Read Data")
    if (args.input == None):
        mat = None
    elif (args.input_format == "dense"):
        mat = np.loadtxt(args.input)
    elif (model.distance == "jaccard"):
        mat = readMatrix(args.input, binary=True)
    else:
        mat = readMatrix(args.input, binary=False)
    n, _ = mat.shape
